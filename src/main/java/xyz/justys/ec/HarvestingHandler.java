...
public final class HarvestingHandler {
    ...
    public static void handleBlockBreak(ServerPlayerInteractionManager manager,
        ...
        try {
            if (shouldTriggerTreeFeller(player, originalState, config)) {
                ...
                if (config.isClearLeavesWithTreeFeller()) {
                    clearAttachedLeaves(world, player, treeBlocks);
                }
    ...
    private static void clearAttachedLeaves(ServerWorld world, ServerPlayerEntity player, Set<BlockPos> treeBlocks) {
        Set<BlockPos> leaves = new LinkedHashSet<>();
        ArrayDeque<BlockPos> queue = new ArrayDeque<>();
        Set<BlockPos> visited = new HashSet<>(treeBlocks);

        for (BlockPos treePos : treeBlocks) {
            for (int[] offset : NEIGHBOR_OFFSETS) {
                BlockPos next = treePos.add(offset[0], offset[1], offset[2]);
                if (visited.add(next)
                        && world.getBlockState(next).isIn(BlockTags.LEAVES)
                        && belongsToBrokenTree(world, next, treeBlocks)) {
                    queue.add(next.toImmutable());
                }
            }
        }

        while (!queue.isEmpty() && leaves.size() < 512) {
            BlockPos current = queue.removeFirst();
            if (!world.getBlockState(current).isIn(BlockTags.LEAVES) || !belongsToBrokenTree(world, current, treeBlocks)) {
                continue;
            }
            leaves.add(current);
            for (int[] offset : NEIGHBOR_OFFSETS) {
        ...
        for (BlockPos leafPos : leaves) {
            ...
            Block.dropStacks(leafState, world, leafPos, world.getBlockEntity(leafPos), player, player.getMainHandStack());
            world.removeBlock(leafPos, false);
        }
    }

    private static boolean belongsToBrokenTree(ServerWorld world, BlockPos leafPos, Set<BlockPos> treeBlocks) {
        boolean nearBrokenTree = false;
        for (BlockPos treePos : treeBlocks) {
            if (Math.abs(treePos.getX() - leafPos.getX()) <= 4
                    && Math.abs(treePos.getY() - leafPos.getY()) <= 6
                    && Math.abs(treePos.getZ() - leafPos.getZ()) <= 4) {
                nearBrokenTree = true;
                break;
            }
        }
        ...
        for (int[] offset : NEIGHBOR_OFFSETS) {
            BlockPos next = leafPos.add(offset[0], offset[1], offset[2]);
            if (treeBlocks.contains(next)) {
                continue;
            }
            if (isTreeMaterial(world.getBlockState(next))) {
                return false;
            }
        }
